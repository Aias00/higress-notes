# 如何让 Higress 对接 Nacos 3 MCP Server 管理功能（SSE 服务版）

## 概述

本文将介绍在 Nacos 3 上配置 MCP Server，将一个支持 SSE Transport 的 MCP Server 通过 Higress 暴露给 MCP Client 访问的完整过程。整个流程将基于一个测试用的简单后端服务实现进行。所有的配置内容也均以此服务所提供的功能为模版编写。建议大家先严格按照本文的步骤配置完成后再尝试自行调整配置对接其他服务。
...
5. 参考下图填写 MCP Server 信息，并发布<br/>
    ![new-mcp-server](../images/mcp-with-nacos3-sse/new-mcp-server.png)
6. 发布后的 MCP Server 列表如下图所示：<br/>
    ![mcp-server-list](../images/mcp-with-nacos3-sse/mcp-server-list.png)
...
4. 点击“创建服务来源”按钮
5. 参考下图填写服务来源信息<br/>
    ![new-service-source](../images/mcp-with-nacos3-sse/new-service-source.png)
    此处要填写的认证信息就是登录 Nacos 控制台的用户名和密码
6. 点击“确定”按钮
7. 点击左侧的“服务列表”，确认列表中出现了我们刚刚创建的 MCP Server<br/>
    ![service-list](../images/mcp-with-nacos3-sse/service-list.png)
    如果没出现，请查看文档最下方的 FAQ
8. 点击左侧的“系统设置”
...
10. 修改 `higress` 配置项中的 `mcpServer` 配置
    ...
    3.  修改后的配置如下图所示<br/>
        ![higress-config](../images/mcp-with-nacos3-sse/higress-config.png)
    4.  点击“提交”按钮
...
2. 正常情况下页面显示如下图所示<br/>
    ![request-sse](../images/mcp-with-nacos3-sse/request-sse.png)
3. 用 curl 命令验证获取工具列表
    ```bash
...
4. 正常情况下不仅终端中会返回工具列表，之前的浏览器页面上也会推送工具列表信息<br/>
    ![request-tools-list-1](../images/mcp-with-nacos3-sse/request-tools-list-1.png)
    ![request-tools-list-2](../images/mcp-with-nacos3-sse/request-tools-list-2.png)
5. 用 curl 命令验证工具调用
    ```bash
    curl http://localhost:8888/mcp/sample?sessionId=b9048bb9-9bed-400c-86a1-516f9f69700c \
    -H 'Content-Type: application/json' \
...
6. 正常情况下不仅终端中会返回调用结果，之前的浏览器页面上也会推送调用结果<br/>
    ![request-tools-call-1](../images/mcp-with-nacos3-sse/request-tools-call-1.png)
    ![request-tools-call-2](../images/mcp-with-nacos3-sse/request-tools-call-2.png)
